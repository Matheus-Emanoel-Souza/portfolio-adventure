...
src/
  ...
  data/                     # profile.ts, projects.ts, skills.ts,
                             # achievements.ts, timeline.ts — única fonte de
...
| `data/timeline.ts`       | Quest Log — educação, experiência, marcos                |
...
entradas de timeline) e guarda em `localStorage` quais seções do hub já foram
...
aventura e pro Quick Mode. Rodar com `npm run test -- --run`.

## Próximos passos sugeridos

1. Preencher `src/data/*.ts` com conteúdo real (bio, projetos, timeline,
   LinkedIn) — a estrutura já está pronta pra isso.
